...
noise_type = 'PN'  # options: 'AN', 'PN', 'BOTH'
savefig = True
...
# the Vcc and the bunch length correspond to coast2-bunch2
PSD_PN_list = list(np.arange(-130, -98, 0.3))
...
ax.plot(np.array(PSD_PN_list), np.array(dey_PN_list)*1e9*beta_0*gamma_0*1e-3*3600, '-', c='k')
...
ax.set_xlabel('Phase noise (dBc/Hz)')
#ax.set_xlabel('Phase noise '+r'$\mathrm{(rad^2/Hz)}$')
...
plt.tight_layout()
if savefig:
    plt.savefig('./figures/dey_vs_PN_PSD_Coast2-Setting2.png')
else:
    plt.show()
plt.close()
